...
internal final class DefaultRoomFactory: Sendable, RoomFactory {
    ...
    internal func createRoom(realtime: any InternalRealtimeClientProtocol, chatAPI: ChatAPI, roomID: String, options: RoomOptions, logger: InternalLogger) async throws(InternalError) -> DefaultRoom {
...
internal actor DefaultRoom: InternalRoom {
    ...
    internal init(realtime: any InternalRealtimeClientProtocol, chatAPI: ChatAPI, roomID: String, options: RoomOptions, logger: InternalLogger, lifecycleManagerFactory: any RoomLifecycleManagerFactory) async throws(InternalError) {
        ...
        reactions = await DefaultRoomReactions(
            channel: internalChannel,
            clientID: clientId,
            roomID: roomID,
            logger: logger
        ...
        presence = await DefaultPresence(
            channel: internalChannel,
        ...
        typing = DefaultTyping(
            ...
            clientID: clientId,
            logger: logger,
            timeout: options.typing.timeout
        )
    }
    ...
    internal func onDiscontinuity(bufferingPolicy: BufferingPolicy) async -> Subscription<DiscontinuityEvent> {
        await lifecycleManager.onDiscontinuity(bufferingPolicy: bufferingPolicy)
    }
}
