...
internal final class DefaultMessageReactions: MessageReactions {
    ...
    internal func delete(from messageSerial: String, params: DeleteMessageReactionParams) async throws(ARTErrorInfo) {
        ...
        if reactionType != .unique, params.reaction == nil {
            throw ReactionsError.unableDeleteReactionWithoutName(reactionType: reactionType.rawValue).toInternalError().toARTErrorInfo()
        }
        do {
        ...
        let eventListener = channel.annotations.subscribe { [clientID, logger] annotation in
            ...
            do {
                ...
                guard let annotationClientID = annotation.clientId else {
                    throw ReactionsError.receivedAnnotationWithoutClientId
                }
    ...
    internal enum ReactionsError: Error {
        internal static let fixedStatusCode = 400

        case unableDeleteReactionWithoutName(reactionType: String)
        case receivedAnnotationWithoutClientId
        ...
        internal var message: String {
            switch self {
            case let .unableDeleteReactionWithoutName(reactionType):
                "Cannot delete reaction of type '\(reactionType)' without a reaction name."
            case .receivedAnnotationWithoutClientId:
                "Received annotation without clientId"
            }
        }
    }
}
