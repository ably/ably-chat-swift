...
internal final class DefaultPresence: Presence {
    ...
    @discardableResult
    internal func subscribe(event: PresenceEventType, _ callback: @escaping @MainActor (PresenceEvent) -> Void) -> SubscriptionHandle {
        implementation.subscribe(event: event, callback)
    }

    @discardableResult
    internal func subscribe(events: [PresenceEventType], _ callback: @escaping @MainActor (PresenceEvent) -> Void) -> SubscriptionHandle {
