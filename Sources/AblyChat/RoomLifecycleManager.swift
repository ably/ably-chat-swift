...
private extension RoomStatus {
    // TODO: Need to revisit which states can have error, now that we're more at the mercy of the underlying SDK
...
internal actor DefaultRoomLifecycleManager: RoomLifecycleManager {
    ...
    // CHA-RL13
    private var isFirstAttach: Bool
    internal var testsOnly_isFirstAttach: Bool {
        isFirstAttach
    }

    // CHA-RL14
    private var isPostExplicitDetach: Bool
    internal var testsOnly_isPostExplicitDetach: Bool {
        isPostExplicitDetach
    }
    ...
    internal init(
        ...
        internal init(
            ...
            testsOnly_isFirstAttach isFirstAttach: Bool? = nil,
            testsOnly_isPostExplicitDetach isPostExplicitDetach: Bool? = nil,
            channel: any InternalRealtimeChannelProtocol,
            logger: InternalLogger,
            clock: SimpleClock
        ) async {
            await self.init(
                ...
                isFirstAttach: isFirstAttach,
                isPostExplicitDetach: isPostExplicitDetach,
                channel: channel,
                logger: logger,
                clock: clock
            )
    ...
    private init(
        ...
        self.isFirstAttach = isFirstAttach ?? false
        self.isPostExplicitDetach = isPostExplicitDetach ?? false
        ...
        let stateChangeSubscription = await channel.subscribeToState()

        // CHA-RL11, CHA-RL12: listen for state changes from our channel
        ...
        listenForStateChangesTask = Task { [weak self] in
            for await stateChange in stateChangeSubscription {
    ...
    #if DEBUG
        /// Supports the ``testsOnly_subscribeToHandledChannelStateChanges()`` method.
        private let stateChangeHandledSubscriptions = SubscriptionStorage<ARTChannelStateChange>()

        /// Returns a subscription which emits the channel state changes that have been handled by the manager.
        ///
        /// A channel state change is considered handled once the manager has performed all of the side effects that it will perform as a result of receiving this state change. Specifically, once:
        ///
        /// - the manager has emitted all pending discontinuity events provoked by the state change
        /// - the manager has performed all status changes provoked by the state change
        internal func testsOnly_subscribeToHandledChannelStateChanges() -> Subscription<ARTChannelStateChange> {
            stateChangeHandledSubscriptions.create(bufferingPolicy: .unbounded)
        }
    #endif

    /// Implements CHA-RL11 and CHA-RL12’s channel state change handling.
    private func didReceiveStateChange(_ stateChange: ARTChannelStateChange) async {
        ...
        if !hasOperationInProgress {
            // CHA-RL11c
            changeStatus(to: .init(channelState: stateChange.current, error: stateChange.reason))
        }
        ...
        case .update, .attached:
            // CHA-RL12b
            if !stateChange.resumed, !isFirstAttach, !isPostExplicitDetach {
                let error = ARTErrorInfo(chatError: ChatError.roomDiscontinuity(cause: stateChange.reason))
