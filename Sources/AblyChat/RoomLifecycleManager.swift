...
internal class DefaultRoomLifecycleManager: RoomLifecycleManager {
    ...
    private var stateChangeListener: ARTEventListener!
    ...
    private init(
        ...
        stateChangeListener = channel.on { [weak self] stateChange in
            self?.didReceiveStateChange(stateChange)
        }
    }
    ...
    deinit {
        ...
        let (stateChangeListener, channel) = (self.stateChangeListener, self.channel)
        Task { @MainActor in
            channel.unsubscribe(stateChangeListener)
        }
    }
    ...
    private func didReceiveStateChange(_ stateChange: ARTChannelStateChange) {
