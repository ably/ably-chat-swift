...
internal final class DefaultTyping: Typing {
    ...
    internal init(featureChannel: FeatureChannel, roomID: String, clientID: String, logger: InternalLogger, heartbeatThrottle: TimeInterval) {
        self.featureChannel = featureChannel
        implementation = .init(featureChannel: featureChannel, roomID: roomID, clientID: clientID, logger: logger, heartbeatThrottle: heartbeatThrottle, gracePeriod: gracePeriod)
    }

    #if DEBUG
        internal init(featureChannel: FeatureChannel, roomID: String, clientID: String, logger: InternalLogger, heartbeatThrottle: TimeInterval, gracePeriod: TimeInterval, typingTimerManager: TypingTimerManagerProtocol) {
            self.featureChannel = featureChannel
            implementation = .init(featureChannel: featureChannel, roomID: roomID, clientID: clientID, logger: logger, heartbeatThrottle: heartbeatThrottle, gracePeriod: gracePeriod, typingTimerManager: typingTimerManager)
        }
    #endif
    ...
    private final class Implementation {
        ...
        internal init(featureChannel: FeatureChannel, roomID: String, clientID: String, logger: InternalLogger, heartbeatThrottle: TimeInterval, gracePeriod: TimeInterval) {
            self.roomID = roomID
            self.featureChannel = featureChannel
            ...
            internal init(featureChannel: FeatureChannel, roomID: String, clientID: String, logger: InternalLogger, heartbeatThrottle: TimeInterval, gracePeriod: TimeInterval, typingTimerManager: TypingTimerManagerProtocol) {
                ...
                self.heartbeatThrottle = heartbeatThrottle
                self.gracePeriod = gracePeriod
                self.typingTimerManager = typingTimerManager
            }
        #endif
        ...
        internal func subscribe(bufferingPolicy: BufferingPolicy) -> Subscription<TypingEvent> {
            ...
            let stoppedEventListener = featureChannel.channel.subscribe(TypingEvents.stopped.rawValue) { [weak self] message in
                ...
                if typingTimerManager.isTypingTimerActive(for: messageClientID) {
                    ...
                    typingTimerManager.cancelTypingTimer(for: messageClientID, isSelf: false)
        ...
        internal func keystroke() async throws(ARTErrorInfo) {
            ...
            do {
                try await keyboardTaskQueue.enqueue { [weak self] in
                    try await self?.publishStartedEvent()
                }
            } catch {
                throw error
            }
