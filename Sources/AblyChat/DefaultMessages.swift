...
private struct MessageSubscriptionWrapper {
    let subscription: MessageSubscriptionHandle
    var serial: String
}
...
internal final class DefaultMessages: Messages {
    ...
    internal func subscribe(_ callback: @escaping @MainActor (Message) -> Void) async throws(ARTErrorInfo) -> MessageSubscriptionHandle {
    ...
    private final class Implementation: Sendable {
        ...
        internal func subscribe(_ callback: @escaping @MainActor (Message) -> Void) async throws(ARTErrorInfo) -> MessageSubscriptionHandle {
            do {
                ...
                let subscriptionHandle = MessageSubscriptionHandle { [weak self, channel] in
                    channel.unsubscribe(eventListener)
                    self?.subscriptionPoints.removeValue(forKey: uuid)
                } getPreviousMessages: { [weak self] queryOptions throws(ARTErrorInfo) in
                ...
                subscriptionPoints[uuid] = .init(subscription: subscriptionHandle, serial: serial)
