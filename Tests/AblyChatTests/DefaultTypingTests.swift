...
struct DefaultTypingTests {
    ...
    private func createTyping(
        ...
        mockFeatureChannel: MockFeatureChannel? = nil
    ) -> DefaultTyping {
        let mockLogger = MockInternalLogger()

        let mockFeatureChannel = mockFeatureChannel ?? MockFeatureChannel(
            channel: channel,
    ...
    func subscribe_ReturnsSubscriptionForTypingEvents() async {
        ...
        // When
        let subscription: Subscription<TypingEvent>? = typing.subscribe()
    ...
    func subscribe_ReceivesStartedTypingEvent() async throws {
        ...
        subscription.emit(TypingEvent(currentlyTyping: [clientId], change: .init(clientId: clientId, type: .started)))
    ...
    func subscribe_ReceivesStoppedTypingEvent() async throws {
        ...
        // When
        let subscription = typing.subscribe()
        subscription.emit(TypingEvent(currentlyTyping: [], change: .init(clientId: clientId, type: .stopped)))
    ...
    func get_ReturnsCurrentlyTypingClients() async throws {
        ...
        let message = ARTMessage(name: TypingEvents.started.rawValue, data: [], clientId: "test-client")
        channel.simulateIncomingMessage(message, for: TypingEvents.started.rawValue)
    ...
    func stop_PublishesCorrectFormatMessage() async throws {
        ...
        // Then
        let message = channel.publishedMessages.last
        #expect(message?.name == TypingEvents.stopped.rawValue)
    ...
    // @spec CHA-T7 - Tests subscribing to discontinuity events
    @Test
    @available(iOS 16.0, tvOS 16.0, *)
    func onDiscontinuity() async throws {
        // Given: A DefaultTyping instance
        let channel = MockRealtimeChannel()
        let featureChannel = MockFeatureChannel(channel: channel)
        let typing = createTyping(mockFeatureChannel: featureChannel)
        ...
        let typingDiscontinuitySubscription = typing.onDiscontinuity()
        featureChannel.emitDiscontinuity(featureChannelDiscontinuity)

        // Then: The DefaultTyping instance emits this discontinuity through `onDiscontinuity`
        let typingDiscontinuity = try #require(await typingDiscontinuitySubscription.first { @Sendable _ in true })
        #expect(typingDiscontinuity == featureChannelDiscontinuity)
    }
