...
struct DefaultRoomTests {
    ...
    func onDiscontinuity() async throws {
        ...
        let roomDiscontinuitiesSubscription = await room.onDiscontinuity()
        await lifecycleManager.emitDiscontinuity(managerDiscontinuity)

        // Then: The room emits this discontinuity through `onDiscontinuity`
        let roomDiscontinuity = try #require(await roomDiscontinuitiesSubscription.first { _ in true })
