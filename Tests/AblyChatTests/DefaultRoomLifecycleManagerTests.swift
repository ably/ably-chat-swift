...
struct DefaultRoomLifecycleManagerTests {
    ...
    private func createManager(
        ...
        forTestingWhatHappensWhenHasIsFirstAttach isFirstAttach: Bool? = nil,
        forTestingWhatHappensWhenHasIsPostExplicitDetach isPostExplicitDetach: Bool? = nil,
        channel: MockRealtimeChannel? = nil,
        clock: SimpleClock = MockSimpleClock()
    ) async -> DefaultRoomLifecycleManager {
        await .init(
            testsOnly_roomStatus: roomStatus,
            testsOnly_isFirstAttach: isFirstAttach,
            testsOnly_isPostExplicitDetach: isPostExplicitDetach,
    ...
    func waitForManager(_ manager: DefaultRoomLifecycleManager, toHandleChannelStateChange stateChange: ARTChannelStateChange, during action: () async -> Void) async {
        let subscription = await manager.testsOnly_subscribeToHandledChannelStateChanges()
    ...
    func attach_whenAlreadyAttached() async throws {
        ...
        let manager = await createManager(forTestingWhatHappensWhenCurrentlyIn: .attached)
        ...
        #expect(await channel.attachCallCount == 0)
    }
    ...
    func attach_attachesChannel_andWhenItAttachesSuccessfully_transitionsToAttached() async throws {
        ...
        let manager = await createManager(
            ...
            forTestingWhatHappensWhenHasIsFirstAttach: true,
            forTestingWhatHappensWhenHasIsPostExplicitDetach: true,
            channel: channel
        )
        ...
        // Then: It calls `attach` on the channel, the room attach operation succeeds, it emits a status change to ATTACHED, its current status is ATTACHED, and it sets the isPostExplicitDetach and isFirstAttach flags to false
        #expect(await channel.attachCallCount > 0)

        _ = try #require(await attachedStatusChange, "Expected status change to ATTACHED")
        try #require(await manager.roomStatus == .attached)

        #expect(await !manager.testsOnly_isPostExplicitDetach)
        #expect(await !manager.testsOnly_isFirstAttach)
    }
    ...
    func attach_whenChannelFailsToAttach() async throws {
        ...
        let manager = await createManager(channel: channel)
    ...
    func detach_whenAlreadyDetached() async throws {
        ...
        let channel = createChannel()
        let manager = await createManager(forTestingWhatHappensWhenCurrentlyIn: .detached, channel: channel)
        ...
        #expect(await channel.detachCallCount == 0)
    }
    ...
    func detach_detachesChannel_andWhenItDetachesSuccessfully_transitionsToDetached() async throws {
        ...
        try #require(await !manager.testsOnly_isPostExplicitDetach)
        ...
        // Then: It calls `detach` on the channel, the room detach operation succeeds, it emits a status change to DETACHED, its current status is DETACHED, and it sets the isPostExplicitDetach flag to true
        #expect(await channel.detachCallCount > 0)

        _ = try #require(await detachedStatusChange, "Expected status change to DETACHED")
        try #require(await manager.roomStatus == .detached)
    ...
    func detach_whenChannelFailsToDetach() async throws {
        ...
        for error in await [failedStatusChange.error, manager.roomStatus.error, roomDetachError] {
    ...
    func release_whenAlreadyReleased() async {
        // Given: A DefaultRoomLifecycleManager in the RELEASED status
        let channel = createChannel()
        let manager = await createManager(forTestingWhatHappensWhenCurrentlyIn: .released, channel: channel)
    ...
    func release_whenDetachedOrInitialized(status: RoomStatus) async throws {
        ...
        let channel = createChannel()
        let manager = await createManager(forTestingWhatHappensWhenCurrentlyIn: status, channel: channel)
    ...
    func release_whenChannelIsNotFailed() async throws {
        ...
        // - the room transitions to RELEASED
        // - the call to `performReleaseOperation()` completes
        #expect(await channel.detachCallCount == 1)
    ...
    func release_whenDetachFails_ifChannelIsNotFailed_retriesAfterPause() async {
        ...
        #expect(await channel.detachCallCount == 3)
    ...
    func release_whenDetachFails_ifChannelIsFailed_doesNotRetry() async {
        ...
        #expect(await clock.sleepCallArguments.isEmpty)
    ...
    // MARK: - Handling channel state change events

    // TODO: understand what was that previous threading concern that I had, and see if still an issue (don't remove waitForManager(manager, toHandleChannelStateChange:) until then) - actually we still need waitToHandle to know if it's emitted any state change that it's going to emit

    // @specOneOf(1/2) CHA-RL11a
    // @spec CHA-RL11b
    @Test
    func channelStateChange_withOperationInProgress() async throws {
        ...
        let originalRoomStatus = await manager.roomStatus

        // When: The channel emits a state change
        // TODO: test with an UPDATE too (I still don't understand fully what the expected behaviour is)
        ...
        await waitForManager(manager, toHandleChannelStateChange: channelStateChange) {
            await channel.emitStateChange(channelStateChange)
        }

        // Then: The manager does not change room status
        #expect(await manager.roomStatus == originalRoomStatus)

        // Post-test: Allow the room lifecycle operation to complete
        channelAttachBehavior.complete(behavior: .success /* arbitrary */ )
    }

    // @specOneOf(2/2) CHA-RL11a
    // @spec CHA-RL11c
    @Test
    func channelStateChange_withNoOperationInProgress() async throws {
        ...
        for roomStatus in await [roomStatusChange.current, manager.roomStatus] {
    ...
    // @spec CHA-RL12b
    @Test(arguments: [
        // TODO: remind myself of the possible realistic values here
        (
            isFirstAttach: false,
            isPostExplicitDetach: false,

            // State change to ATTACHED, resumed false
            channelStateChange: ARTChannelStateChange(
                current: .attached,
        ...
        (
            ...
            // UPDATE event, resumed true (so ineligible for a discontinuity)
            channelStateChange: ARTChannelStateChange(
                current: .attached, // arbitrary
                previous: .attached,
                event: .update,
        ...
        (
            ...
            // State change to ATTACHED, resumed false (i.e. an event eligible for a discontinuity, but which will be excluded because of isFirstAttach)
            channelStateChange: ARTChannelStateChange(
                current: .attaching, // arbitrary
                previous: .attached,
                event: .attaching,
        ...
        (
            isFirstAttach: false,
            isPostExplicitDetach: true,

            // State change to ATTACHED, resumed false (i.e. an event eligible for a discontinuity, but which will be excluded because of isPostExplicitDetach)
            channelStateChange: ARTChannelStateChange(
                current: .attaching, // arbitrary
                previous: .attached,
                event: .attaching,
    ...
    func channelStateChange_discontinuity(
        isFirstAttach: Bool,
        isPostExplicitDetach: Bool,
        channelStateChange: ARTChannelStateChange,
        ...
        let channel = createChannel()
        let manager = await createManager(
            forTestingWhatHappensWhenHasIsFirstAttach: isFirstAttach,
            forTestingWhatHappensWhenHasIsPostExplicitDetach: isPostExplicitDetach,
            channel: channel
        )

        let discontinuitiesSubscription = await manager.onDiscontinuity(bufferingPolicy: .unbounded)
        ...
        if expectDiscontinuity {
            ...
            #expect(
                isChatError(
                    ...
                    cause: channelStateChange.reason
                )
            )
        } else {
